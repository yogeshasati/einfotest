...
int main()
{
    ...
    int eth1_sock_raw = socket( AF_PACKET , SOCK_RAW , htons(ETH_P_ALL)) ; //For receiving
    int eth0_sock     = socket( PF_PACKET , SOCK_RAW , IPPROTO_RAW) ;      //For sending
	...
	if (bind(eth1_sock_raw, (struct sockaddr*) &saddr, sizeof(saddr)) < 0) {
    ...
    if (setsockopt(eth0_sock, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr)) < 0)
	{
        perror("bind to eth0 failed..");
    }
	else
		printf("eth0 setsockopt successfull\n");
    ...
    while(1)
    {
		...
		//Receive a packet
        data_size = recvfrom(eth1_sock_raw , buffer , 65536 , 0 ,(struct sockaddr *) &saddr , (socklen_t*)&saddr_size);

        if(data_size <0 )
        {
            printf("Recvfrom error , failed to get packets from eth1..\n");
            return 1;
        }
        else
		{
